...
export default function CardVendaAvulsa() {
  ...
  const handleClosePedido = (index) => {
    ...
    const itemsToUpdate = updatedVendas[index].items;
    ...
    Object.keys(itemCountMap).forEach(nome => {
      ...
      axios.get(`http://localhost:5000/estoque/${nome}`)
        .then(response => {
          ...
          const novaQuantidade = Math.max(0, quantidadeAtual - quantidadeParaSubtrair);
  ...
  return (
    <div className="flex flex-wrap gap-4">
      {vendas.map((venda, index) => {
        const valorTotalVenda = venda.items.reduce((total, item) => total + item.valor, 0);
